...
class CategoriesLandingViewHolder(itemView: View,var gamingLogCallbacks: GamingLogCallbacks):RecyclerView.ViewHolder(itemView) {
    fun bind(list: GameList, adsBundle: Bundle) {
        ...
        categoryRecyclerView.adapter= CategoriesLandingAdapter(list.games,gamingLogCallbacks)
    ...
    private fun logGameCategory(category: String?) {
        if(gamingLogCallbacks!=null){
            if (category != null) {
                gamingLogCallbacks.onCategoryClicked(category)
            }
        }
    }
}